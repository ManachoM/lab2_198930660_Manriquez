...
checkScene(Scene):- escenario(_,_,_,_,_,_,Scene) ; (nth0(1, Scene, N), nth0(2, Scene, M), nth0(3, Scene, Eq1), nth0(4, Scene, Eq2),N>0, M>0, append(X1, X2, L), generarX(Eq1, X1), generarX(Eq2, X2), length(Eq1, L1), length(Eq2, L2),  not(seRepite(L)), M>=L1+L2).
